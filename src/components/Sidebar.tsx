...
const Sidebar: React.FC<SidebarProps> = ({ activeItem, onSelect }) => {
  const sections = [
    {
      title: 'FEED',
      items: ['All News', 'Trending']
    },
    {
      title: 'CATEGORIES',
      items: ['Crime', 'Politics', 'Events', 'Jobs']
    },
    {
      title: 'DASHBOARD',
      items: ['My Dashboard', 'Admin Panel']
    },
    {
      title: 'ACCOUNT',
      items: ['Profile', 'Journalists', 'Notifications', 'Bookmarks', 'Settings']
    }
  ]
  ...
  return (
    <aside className="hidden h-dvh w-72 flex-col border-r border-slate-800 bg-slate-950 text-slate-100 lg:flex">
        ...
        <div className="space-y-7">
          {sections.map((section) => (
            <div key={section.title}>
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-slate-500">
                {section.title}
              </p>
                  ...
                  return (
                    <li key={item}>
                      <button
                        ...
                        className={
                          `group relative flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left text-sm font-medium outline-none transition focus-visible:ring-2 focus-visible:ring-sky-400/60 focus-visible:ring-offset-0 ${
                            isActive
                              ? 'bg-white/10 text-sky-200 ring-1 ring-white/10 shadow-[0_12px_30px_-18px_rgba(0,0,0,0.9)]'
                          ...
                          className={
                            `inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border ${
                              isActive
                                ...
                                : 'border-white/5 bg-white/5 text-slate-300 group-hover:border-white/10'
                            }`
                          }
                        >
      ...
      <div className="border-t border-white/5 px-6 py-5">
        ...
        >
          <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-sm font-semibold text-slate-100">
            G
          </span>
          <span className="min-w-0">
            <span className="block truncate text-sm font-semibold text-slate-100">Guest User</span>
            <span className="block truncate text-xs text-slate-400">Sign in to post news</span>
          </span>
        </button>
      </div>
    </aside>
